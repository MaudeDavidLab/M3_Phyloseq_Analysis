...
output:
  html_document:
    css: /Users/shokoiwai/styles.css
...
# M3 google drive location
M3folder_loc='G:/Shared drives/M3-shared/'
# M3 repo location
m3_repo='/Users/shokoiwai/Bitbucket/M3/'
##########

## Scripts folder
path_to_use=paste0(m3_repo, 'Scripts/')

## files for sequencing summary -> this will generate all counts tables.
# A text file that has all relative paths (from M3 folder) to sample sheet files
sample_sheet_list_fp='Data/V4/00_SeqSummary/180809_SampleSheet_list.txt'

## Read counts data
# A text file that has all relative paths (from M3 folder) to DD results directories
DDreslist_fp='Data/V4/00_SeqSummary/180809_DDresults_list.txt'

#File names changed, this file maps old file names to new ones
asteria_mappings_fp = 'Data/V4/00_seqSummary/200312_Asteria_mapping.txt'

## Metadata
# RC Download from DAPPER, KB https://kb.secondgenome.com/detail/api/lims/reportcollection/detail/87/ on 8/23/18
# this should give mapping between biospecimen and omics file sets
map_fp='Data/V4/00_Metadata/INRD17_0415 (M3) 16S_NGS 180831.csv'
# metadata table Downloaded from DAPPER, INRD17_0415 on 8/31/18
metadata_fp='Data/V4/00_Metadata/SG_SIwai_INRD17_0415_downldAug31.csv'
# metadata category file, this file specifies dietary questions
meta_cat_fp='Data/V4/00_Metadata/metadata_categories.csv'
...
16S rRNA amplicon sequencing (minimum 20,000 reads per sample)
## Summary sequencing results
```{r seq_depth}
# read Sample Sheet file path
SSlist=fread(paste0(M3folder_loc, sample_sheet_list_fp), header=FALSE)

# read each file and add to the master
all_map=NULL
for(i in 1:nrow(SSlist)){
  batch_name=as.character(SSlist[i, V1])
  ss_data=fread(paste0(M3folder_loc, as.character(SSlist[i, V2])), header=TRUE)
  # select columns
  ss_data <- ss_data[, list(Library_ID=Sample_ID, Description, ReadDepth=`READ COUNT`, family)]
  # add batch name
  ss_data <- ss_data[, batch:=batch_name]
  # add to master
  all_map=rbindlist(list(all_map, ss_data))
}

# remove all 'Undetermined' (PhiX) and controls
all_map <- all_map[grepl('M3_', family)]

# replace '-' to '_' in the sequencing batch
all_map <- all_map[, batch:=gsub('-', '_', batch)]

# count sample numbers
uniqueN(all_map$Description)

# plot read distribution in each batch
# add pseudocounts of 0.0001 to ReadDepth==0 samples for plotting
ggplot(data=all_map[, list(batch, RD=ifelse(ReadDepth==0, 0.0001, ReadDepth))], aes(x=batch, y=RD))+
  geom_boxplot(outlier.color='white')+
  geom_jitter(position=position_jitter(width=0.4), size=1)+
  scale_y_log10()+
  theme_bw()+ ylab('Read Depth')+
  theme(axis.text.x=element_text(angle=45, hjust=1))
```

## Asteria (CLAMPR-DADA2) results
```{r}
# read DD results file path
DDreslist=fread(paste0(M3folder_loc, DDreslist_fp), header=FALSE)

asteria_mapping = fread(paste0(M3folder_loc, asteria_mappings_fp), header = FALSE)

# read each file and add to the master
all_cts=NULL
all_tax_data=NULL

for(i in 1:nrow(DDreslist)){
  # record batch
  batch_name=as.character(DDreslist[i, V1])
  print(batch_name)
  
  # read taxonomy table
  folder = asteria_mapping$V5[asteria_mapping$V4 == as.character(DDreslist[i, V2])]
  tax_data=fread(paste0(M3folder_loc, folder, '700_strainselect_taxtable.csv'), header=TRUE)
 
  # read abundance table
  dd_data=fread(paste0(M3folder_loc, folder, '400_DADA_ASVabundance_withSequence.csv'), header=TRUE)
  
  # add ASV seq to taxonomy table
  tax_data <- merge(tax_data, dd_data[, list(ASV, Sequence)], by='ASV')
  # remove column 'ASV' and 'bin' from taxonomy table
  tax_data <- tax_data[, c('ASV', 'bin'):=NULL]
  # add to master
  all_tax_data <- unique(rbindlist(list(all_tax_data, tax_data)))
  
  # remove column 'ASV' from abundance table
  dd_data <- dd_data[, ASV:=NULL]
  # convert to long format
  dd_data <- melt(dd_data, id.vars='Sequence')
  # remove 0
  dd_data <- dd_data[value!=0]
  # add batch name
  dd_data <- dd_data[, batch:=batch_name]
  # add to master
  all_cts=rbindlist(list(all_cts, dd_data))
}

# replace column name
setnames(all_cts, 'variable', 'Library_ID')

# replace '-' to '_' in the sequencing batch
all_cts <- all_cts[, batch:=gsub('-', '_', batch)]
```

### Number of unique ASVs
```{r unique_ASVs}
# summary unique ASVs in each batch
pander(all_cts[, list(uniqASV=uniqueN(Sequence)), by='batch'])
```

### Read length distribution
```{r read_len_distribution}
# read length distribution
# add sequence length
all_cts <- all_cts[, asvLen:=nchar(Sequence)]

pander(all_cts[, list(min_len=min(asvLen), max_len=max(asvLen), ave_len=mean(asvLen), median_len=median(asvLen)), by='batch'])
```

### Seq depth to post DD counts
```{r depth_postDD}
pre_post_dd <- merge(all_cts[, list(post_DD_cts=sum(value)), by=c('Library_ID', 'batch')],
                     all_map[, list(Library_ID, batch, ReadDepth)],
                     by=c('Library_ID', 'batch'))

# add percent remained
pre_post_dd <- pre_post_dd[, RatioPostDD:=post_DD_cts/ReadDepth*100]

ggplot(data=pre_post_dd, aes(x=ReadDepth, y=RatioPostDD, colour=batch))+
  geom_point()+
  scale_x_log10()+
  ylab('Remained % postDD')+
  theme_bw()

# stats table
pre_post_dd[is.na(pre_post_dd)] <- 0
pander(merge(pre_post_dd[, list(LibraryN=uniqueN(Library_ID)), by='batch'],pre_post_dd[, list(min_depth=min(ReadDepth), max_depth=max(ReadDepth), mean_depth=mean(ReadDepth), median_depth=median(ReadDepth),
                   min_postDD=min(post_DD_cts), max_postDD=max(post_DD_cts), mean_postDD=mean(post_DD_cts), median_postDD=median(post_DD_cts)), by='batch'],  by='batch'), split.table=Inf)
```

### Merge all batches
```{r merged_cts}
# add sample description to counts table
all_cts <- merge(all_cts, all_map[, list(Library_ID, Description, family, batch)], by=c('Library_ID', 'batch'), all=FALSE) # need an intersect to remove all no-reads samples and controls
# number of unique ASVs
uniqueN(all_cts$Sequence)
# number of unique samples - this is less than the number in all_map as some samples did not produce any counts and removed here.
uniqueN(all_cts$Description)

# clean-up counts table for downstream use
all_cts=all_cts[, list(Sequence, OmicsFileSet.Barcode=paste0(Library_ID, '_', batch), value)]
```
...
## Read metadata
```{r read_meta}
mapDT=fread(paste0(M3folder_loc, map_fp), header=TRUE)    # this file is only used to map Biospecimen.Barcode and OmicsFileSet.Barcode
metaDT=fread(paste0(M3folder_loc, metadata_fp), header=TRUE)    # this file is used to get all metadata for the Biospecimen.Barcode

# Add phenotype column to metaDT
metaDT$phenotype <- as.character(lapply(strsplit(as.character(metaDT$Host.Name), split='_'), '[', 2))
```

## Make Phyloseq object
```{r generate_phyloseq}
dir.create(paste0(output_data, 'making_phyloseq/'))

## M3 specific ASV ids
# for easy printing, convert all unique ASVs to IDs
asv_map <- data.table(ASVid=paste0('ASV_', 1:uniqueN(all_cts$Sequence)), Sequence=unique(all_cts$Sequence))
# save it!
write.csv(asv_map, file=paste0(output_data, 'making_phyloseq/ASVid_map_', Sys.Date(), '.csv'), row.names=FALSE)

# add ASVid to tax table and save
all_tax_data <- merge(all_tax_data, asv_map, by='Sequence', all.x=TRUE)

# write tax file
write.csv(all_tax_data, file=paste0(output_data, 'all_tax_', Sys.Date(), '.csv'), row.names=FALSE)

# add ASVid to counts table
all_cts <- merge(all_cts, asv_map, by='Sequence')

## Add Biospecimen ID to counts table
all_cts <- merge(all_cts[, list(ASVid, OmicsFileSet.Barcode, value)], mapDT[, list(OmicsFileSet.Barcode, Biospecimen.Barcode)], by='OmicsFileSet.Barcode', all.x=TRUE)

## Remove samples that do not pass min_postDD
pass_min_postDD_cts <- all_cts[Biospecimen.Barcode %in% (all_cts[, list(post_DD_cts=sum(value)), by='Biospecimen.Barcode'][post_DD_cts>min_postDD]$Biospecimen.Barcode)]
# number of samples passed
uniqueN(pass_min_postDD_cts$Biospecimen.Barcode)

## Now merge data by sample -> this is the valid sample counts table
pass_min_postDD_cts <- pass_min_postDD_cts[, list(cts=sum(value)), by=c('Biospecimen.Barcode', 'ASVid')]

## Subset metadata based on valid samples
pass_min_postDD_cts_meta <- metaDT[Biospecimen.Barcode %in% pass_min_postDD_cts$Biospecimen.Barcode]

## Finally make them into a phyloseq object
# OTU table
OTU=dcast(pass_min_postDD_cts, ASVid ~ Biospecimen.Barcode, value.var='cts')
OTU <- data.frame(OTU)
row.names(OTU) <- OTU$ASVid
OTU$ASVid <- NULL
OTU[is.na(OTU)] <- 0
# use ASV sequence as taxonomy
TAX=as.matrix(asv_map[ASVid %in% pass_min_postDD_cts$ASVid])
row.names(TAX) <- TAX[, 'ASVid']
# metadata
SAMPLEDATA <- data.frame(pass_min_postDD_cts_meta)
row.names(SAMPLEDATA) <- SAMPLEDATA$Biospecimen.Barcode
# phyloseq object
ps_not_norm = phyloseq(otu_table(OTU, taxa_are_rows=TRUE), tax_table(TAX), sample_data(SAMPLEDATA))

#and now save this object
saveRDS(ps_not_norm, file=paste(output_data, "making_phyloseq/ps_not_norm_pass_min_postDD.Rda", sep=""))

# remove samples with age <24 mo, except 004_N and 065_N
# make sure age is numeric
sample_data(ps_not_norm)$Age..months. <- as.numeric(sample_data(ps_not_norm)$Age..months.)
# find samples to keep
keep_id=(get_variable(ps_not_norm, 'Age..months.')>23 
             | get_variable(ps_not_norm, 'Host.Name') %in% c('004_N', '065_N'))

ps_not_norm <- prune_samples(keep_id, ps_not_norm)

#And now how many complete families? 
complete_families=unique(sample_data(ps_not_norm)$Family.group.ID)[table(sample_data(ps_not_norm)$Family.group.ID) == 6]
length(complete_families)

# subset phyloseq object by complete families
ps_not_norm_comp <- prune_samples(row.names(sample_data(ps_not_norm)[sample_data(ps_not_norm)$Family.group.ID %in% complete_families]), ps_not_norm)

# generated phyloseq object
ps_not_norm_comp

# save 
saveRDS(ps_not_norm_comp, file=paste(output_data, "making_phyloseq/ps_not_norm_comp_pass_min_postDD.Rda", sep=""))

rm(OTU, SAMPLEDATA, TAX)
```
...
datatable(all_tax_data[ASVid %in% significant_tax$rn, list(ASVid, Strain, ss_id, Kingdom, Phylum, Class, Order, Family, Genus, Species)])
```
...
permanova_var=meta_cat[permanova!=FALSE]
...
ps_pcoa <- ordinate(
  ...
  method = "CAP", 
  distance = "bray",
  formula = ~ Vegetable..consumption.frequency. + Vegetable..consumption.frequency...longitudinal. +
    Dairy..consumption.frequency. + Fruit..consumption.frequency. + Toilet.cover..M3. + Dairy..consumption.frequency...longitudinal. + Meats.and.seafood..consumption.frequency...longitudinal. + Non.celiac.gluten.sensitivity + Dietary.restrictions..M3. + GI.symptoms.within.3.months..M3. + Multivitamin + Biological.sex + GI.issues.this.week..M3. + Lactose.intolerance + Other.symptoms.this.week..M3.
)
