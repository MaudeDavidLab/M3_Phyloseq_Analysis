...
write_csv(metadata_ok, "sam_data.csv")
...
### Slope test on numerical variables

```{r slope test}

write_csv(metadata_ok, "sam_data2.csv")
...
## ANCOM
```{r ancom}
#Run ANCOM_old ver (2.1 can be seen starting on line595)
#retrive taxa
#format metadata
...
#Format the Detected Table w/ Taxa of all AR vs. ALL Con
...
otu_data2 = read_tsv("~/ANCOM2.1/ANCOM/data/ecam-table-taxa.tsv", skip = 1)
otu_id = otu_data2$`feature-id`
otu_data2 = data.frame(otu_data2[, -1], check.names = FALSE)
rownames(otu_data) = otu_id

meta_data2 = read_tsv("~/ANCOM2.1/ANCOM/data/ecam-sample-metadata.tsv")[-1, ]
meta_data2 = meta_data2 %>% rename(Sample.ID = `#SampleID`)

# Step 1: Data preprocessing

feature_table2 = otu_data2; sample_var = "Sample.ID"; group_var = "delivery"
out_cut = 0.05; zero_cut = 0.90; lib_cut = 0; neg_lb = TRUE
prepro = feature_table_pre_process(feature_table2, meta_data2, sample_var, group_var, 
                                   out_cut, zero_cut, lib_cut, neg_lb)
feature_table2 = prepro$feature_table # Preprocessed feature table
...
t_start = Sys.time()
res = ANCOM(feature_table2, meta_data2, struc_zero, main_var, p_adj_method, 
