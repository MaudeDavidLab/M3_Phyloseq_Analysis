...
num_cat <- num_cat[-which(num_cat %in% c("Language.ability.and.use", "Conversation.ability", "Understands.speech", "Mobile.Autism.Risk.Assessment.Score", "Number.of.small.pet.herbivores", "Number.of.small.pet.rodents", "phenotype_num"))]
...
#doing the same but w/o avg or total just raw values
tmpps <- prune_samples((ps_DeSeq_norm_pass_min_postDD_sup003@sam_data$Family.group.ID == "1"), ps_DeSeq_norm_pass_min_postDD_sup003)
tmppsA <- prune_samples(tmpps@sam_data$phenotype == "A", tmpps)
tmppsN <- prune_samples(tmpps@sam_data$phenotype == "N", tmpps)
...
tab<-tibble(distanceA, distanceN, rep(tmpps@sam_data$Family.group.ID[1], length(distanceA)))
colnames(tab) <- c("DistanceAut", "DistanceNeu", "Family")
...
tabtmp<-tibble(distanceA, distanceN, rep(tmpps@sam_data$Family.group.ID[1], length(distanceA)))
colnames(tabtmp) <- c("DistanceAut", "DistanceNeu", "Family")
...
#tab
# run tests to check significance
taba<-tibble(tab$DistanceAut, rep("A", length(tab$DistanceAut)))
colnames(taba) <- c("Distances_btwnTimepoints","phenotype" )

tabn<-tibble(tab$DistanceNeu, rep("N", length(tab$DistanceNeu)))
colnames(tabn) <- c("Distances_btwnTimepoints","phenotype" )
...
# run tests to check significance
shapiro.test(finaltab3$Distances_btwnTimepoints) #not normal we need a reanking test
wilcox.test(Distances_btwnTimepoints ~ phenotype, data=finaltab3, var.equal=FALSE)
...
A<-distance(tmppsA, "bray", type = "samples")
distanceA=var(c(A[1],A[2],A[3]))

N<-distance(tmppsN, "bray", type = "samples")
distanceN=var(c(N[1],N[2],N[3]))

tab<-tibble(distanceA, distanceN, rep(tmpps@sam_data$Family.group.ID[1], length(distanceA)))
colnames(tab) <- c("DistanceVarAut", "DistanceVarNeu", "Family")
...
tabtmp<-tibble(distanceA, distanceN, rep(tmpps@sam_data$Family.group.ID[1], length(distanceA)))
colnames(tabtmp) <- c("DistanceVarAut", "DistanceVarNeu", "Family")
...
#tab
# run tests to check significance
taba<-tibble(tab$DistanceVarAut, rep("A", length(tab$DistanceVarAut)))
colnames(taba) <- c("DistanceVar_btwnTimepoints","phenotype" )

tabn<-tibble(tab$DistanceVarNeu, rep("N", length(tab$DistanceVarNeu)))
colnames(tabn) <- c("DistanceVar_btwnTimepoints","phenotype" )

finaltab4<-rbind(tabn, taba)
p <- ggplot(finaltab4, aes(x=phenotype, y=DistanceVar_btwnTimepoints)) + 
  geom_boxplot()
p + geom_jitter(shape=16, position=position_jitter(0.2))

# run tests to check significance
shapiro.test(finaltab4$DistanceVar_btwnTimepoints) #not normal we need a reanking test
wilcox.test(DistanceVar_btwnTimepoints ~ phenotype, data=finaltab4, var.equal=FALSE)
...
tmp<-slope.test(otus[,1], ps_DeSeq_norm_pass_min_postDD_sup003@sam_data[,confound], method = 0)

slopetesttab <- tibble(colnames(otus)[1], tmp$p)
colnames(slopetesttab) <- c("otu", "p_val")

for (i in 2:length(colnames(otus))){
    tmp<-slope.test(otus[,i], ps_DeSeq_norm_pass_min_postDD_sup003@sam_data[,confound], method = 0)
...
for (x in 2:length(num_confounds)){
  confound <- num_confounds[x]  
  otus<-as.data.frame(otu_table(ps_DeSeq_norm_pass_min_postDD_sup003))
  otus <- t(otus)
  tmp<-slope.test(otus[,1], ps_DeSeq_norm_pass_min_postDD_sup003@sam_data[,confound])
  ...
  slope_p.val<-p.adjust(slopetesttab$p_val)
  tmp<-slopetesttab$otu[slope_p.val < 0.05]
  slope_sig_asvs_p.val<-c(slope_sig_asvs_p.val, tmp)
...
num_confounds2 <- sig_numvar$Var[-which(sig_numvar$Var %in% c("LR6.probability.not.ASD..M3."))]
