...
#round off year
ps_not_norm_comp@sam_data$Age..years.<-as.numeric(round(ps_not_norm_comp@sam_data$Age..years.))
...
fac_cat<-fac_cat[-which(fac_cat %in% c("Behavior.video.submitted..M3.","Language.ability.and.use","Conversation.ability","Understands.speech","Plays.imaginatively.when.alone","Plays.imaginatively.with.others","Plays.in.a.group.with.others","Eye.contact.finding","Childhood.behavioral.development.finding","Repetitive.motion","Picks.up.objects.to.show.to.others","Sleep.pattern.finding","Response.to.typical.sounds","Self.injurious.behavior.finding","Gastrointestinal.problems..M3.", "Imitation.behavior", "Other.stool.sample.collection.method.explained..M3.", "Flu.shot.in.the.last..MFlu.shot.in.the.last..M3.", "Pica.disease", "Additional.info.affecting.microbiome..M3.", "Dietary.restrictions.details..M3.", "Pet.bird", "Stool.sample.challenges.explained..M3."))]

sample_timepoint_dependent_factors <- c( "GI.issues.this.week..M3.", "Stool.sample.collection.method..M3.", "Problems.collecting.stool.sample..M3.", "Recently.ill" , "Season" , "Fever", "Travel.to.country", "Other.symptoms.this.week..M3.", "Small.pet.rodent")


#same across phenotype due to same household between siblings
equivalent_between_phenotype_due_to_household <- c("Annual.household.income", "Source.of.drinking.water", "Pets.in.home", "Pet.dog", "Pet.cat", "Pet.fish", "Pet.horse", "Pet.reptile", "Indoors.pet", "Outdoors.pet", "Indoors.pets", "Outdoors.pets")

#Other variables that are same across phenotype in this dataset
other_same_var_across_pheontype<- c("Racial.group", "Born.by.caesarian.section", "Environmental.tobacco.smoke.exposure", "Born.by.caesarian.section" )
...
chi_list<-chi_list[-which(chi_list %in% c("Behavior.video.submitted..M3.","Language.ability.and.use","Conversation.ability","Understands.speech","Plays.imaginatively.when.alone","Plays.imaginatively.with.others","Plays.in.a.group.with.others","Eye.contact.finding","Childhood.behavioral.development.finding","Repetitive.motion","Picks.up.objects.to.show.to.others","Sleep.pattern.finding","Response.to.typical.sounds","Self.injurious.behavior.finding","Gastrointestinal.problems..M3.", "Imitation.behavior", "Other.stool.sample.collection.method.explained..M3.", "Flu.shot.in.the.last..MFlu.shot.in.the.last..M3.", "Pica.disease", "Additional.info.affecting.microbiome..M3.", "Dietary.restrictions.details..M3.", "Pet.bird", "Host.disease.status", "phenotype", "LR6.prediction..M3.", "LR10.prediction..M3.", "LR5.prediction..M3.", "Biospecimen.Description",  "Biospecimen.Date.Collected", "Within.study.sampling.date", "Family.group.ID", "Natural.sibling.included.in.present.study" , "Travel.outside.US..notes."  ,"Sequencing.run.ID..OmicsFileSet._collapsed", "Most.recent.GI.episode.symptoms..M3.", "Frequency.of.interaction.with.pet.s.", "Explain.how.you.collected.stool.sample..M3." ,"Stool.sample.challenges.explained..M3."))]

#removing variables that are constant between families

chi_list_constant <- chi_list<-chi_list[-which(chi_list %in% sample_timepoint_dependent_factors)]
chi_list_constant <- chi_list_constant<-chi_list_constant[-which(chi_list_constant %in% equivalent_between_phenotype_due_to_household)]
chi_list_constant <- chi_list_constant<-chi_list_constant[-which(chi_list_constant %in% other_same_var_across_pheontype)]
...
chisquare_p.val_all <- chisquare_p.val
...
chisquare_p.val_all2 <- chisquare_p.val2
...
library(stats)
#mcnemar_test(anx)
anxhigh<-stats::mcnemar.test(highanx)
anxhigh
...
map$fam_and_time <- paste0(map$Family.group.ID, map$Within.study.sampling.date)
map_num<-sapply(map, is.numeric)
num_cat <- colnames(map[,as.vector(which(map_num == TRUE))])
num_cat <- num_cat[-c(1:3)]
num_cat <- num_cat[-which(num_cat %in% c("Language.ability.and.use", "Conversation.ability", "Understands.speech", "Mobile.Autism.Risk.Assessment.Score", "Number.of.small.pet.herbivores", "Number.of.small.pet.rodents", "phenotype_num", "Number.of.pet.birds", "LR6.probability.not.ASD..M3." ,"LR6.probability.ASD..M3." ,"LR10.probability.not.ASD..M3." ,"LR10.probability.ASD..M3.", "LR5.probability.not.ASD..M3.", "LR5.probability.ASD..M3.", "Behavior.video.submitted..M3.","Language.ability.and.use","Conversation.ability","Understands.speech","Plays.imaginatively.when.alone","Plays.imaginatively.with.others","Plays.in.a.group.with.others","Eye.contact.finding","Childhood.behavioral.development.finding","Repetitive.motion","Picks.up.objects.to.show.to.others","Sleep.pattern.finding","Response.to.typical.sounds","Self.injurious.behavior.finding","Gastrointestinal.problems..M3.", "Imitation.behavior", "Other.stool.sample.collection.method.explained..M3.", "Flu.shot.in.the.last..MFlu.shot.in.the.last..M3.", "Pica.disease", "Additional.info.affecting.microbiome..M3.", "Dietary.restrictions.details..M3.", "Pet.bird", "Host.interaction.with.pets" ))]

num_cat_timepoint_dependent <- c(dict_1_items, "Length.of.recent.foreign.travel", "Recent.anxiety..caretaker.reported.")
num_cat_all <- num_cat
num_cat <- num_cat[-which(num_cat %in% num_cat_timepoint_dependent)]
#same between cohorts
num_cat <- num_cat[-which(num_cat %in% c("Natural.father.current.age..months." , "Natural.mother.current.age..months." , "Number.of.roommates", "Number.of.pet.dogs" , "Number.of.pet.cats" , "Number.of.pet.fish" , "Number.of.pet.horses" ,  "Number.of.pet.reptiles", "Foreign.travel.within.past.months"))]

#Testing to see if numerical variables differ in means using wilcox or t-tests
numerical_test_btwn_pheno <- NULL
for (i in num_cat ){
  tmp<-shapiro.test(map[,i])
  tmpmap <- map[which(map$Within.study.sampling.date == "Timepoint 1"),]
  if (tmp$p.value <= 0.05) {
        res<-wilcox.test(as.formula(paste(i, "~ phenotype")), data=tmpmap, var.equal = FALSE)
    ...
    res<-t.test(as.formula(paste(i, "~ phenotype")), data=tmpmap)
...
sig_numvar<-numerical_test_btwn_pheno
sig_numvar$p.value <- p.adjust(sig_numvar$p.value, method = "fdr")

sig_numvar_all <- sig_numvar

#Testing to see if numerical variables differ in means using wilcox or t-tests with others 

num_cat<-num_cat_all[num_cat_all %in% num_cat_timepoint_dependent]
#Testing to see if numerical variables differ in means using wilcox or t-tests with others 
numerical_test_btwn_pheno <- NULL
for (i in num_cat) {
  #map[,c(i, "Within.study.sampling.date", "phenotype", "Biospecimen.Barcode")] %>%
  #group_by(Within.study.sampling.date, phenotype) %>%
  #shapiro_test(i)
  if (sum(is.na(map[,i])) != 0){
    tmpmap <- map[-which(map$Family.group.ID %in% c(map$Family.group.ID[is.na(map[,i])])),]
    }
  tmpmap <- tmpmap[,c(i, "Within.study.sampling.date", "phenotype", "Host.Name")]
  tmpmap$phenotype <- as.factor(tmpmap$phenotype)
  
  #checks for NA removal
  #sum(is.na(tmpmap$Meats.and.seafood..consumption.frequency...longitudinal.))
  #table(tmpmap$Within.study.sampling.date, tmpmap$phenotype)

  #without wrapper formula
  #res.aov <-aov(as.formula(paste0(i,"~ phenotype * Within.study.sampling.date + #Error(Host.Name/(Within.study.sampling.date))")), data = tmpmap)
  
 # summary(res.aov)
  
  #with wrapper
  res.aov <- anova_test(
  data = tmpmap, dv = i, wid = Host.Name,
  between = phenotype, within = Within.study.sampling.date
  )
  res<-get_anova_table(res.aov)
  
  res$testvariable <- i
  numerical_test_btwn_pheno <-rbind(numerical_test_btwn_pheno, res)
}

res_table<-tibble(numerical_test_btwn_pheno$testvariable, numerical_test_btwn_pheno$p, paste0(numerical_test_btwn_pheno$Effect, rep("Two-way Mixed repeated measures Anova", nrow(numerical_test_btwn_pheno))))
colnames(res_table) <- c("Var", "p.value", "Type")
res_table$p.value <- p.adjust(res_table$p.value, method = "fdr")

sig_numvar_all2 <- res_table

sig_numvar2<-res_table[which(res_table$p.value <= 0.05),]

sig_numvar<-rbind(sig_numvar2, sig_numvar)

sig_numvar<-sig_numvar[which(sig_numvar$p.value <= 0.05),]
...
chi1<-tibble(names(chisquare_p.val_all), chisquare_p.val_all, rep("Chi-Square", length(names(chisquare_p.val_all))))
chi2<-tibble(names(chisquare_p.val_all2), chisquare_p.val_all2, rep("Chi-Square", length(names(chisquare_p.val_all2))))
colnames(chi1) <- c("Variable", "q_value", "Type")
colnames(chi2) <- c("Variable", "q_value", "Type")
colnames(sig_numvar_all) <- c("Variable", "q_value", "Type")
colnames(sig_numvar_all2) <- c("Variable", "q_value", "Type")
alltable<-rbind(chi1, chi2, sig_numvar_all, sig_numvar_all2)

write.csv(alltable, "../results/Supplementary_Table_9.csv")

alltable[alltable$q_value<0.05,]

```
### General Demographic Information tables
```{r}

ps_not_norm_time1 <- subset_samples(ps_not_norm, Within.study.sampling.date == "Timepoint 1")
ps_not_norm_time1_asd <- subset_samples(ps_not_norm_time1, phenotype == "A")
ps_not_norm_time1_td <- subset_samples(ps_not_norm_time1, phenotype == "N")
ps_not_norm_comp_asd<-subset_samples(ps_not_norm_comp, phenotype == "A")
ps_not_norm_comp_td<-subset_samples(ps_not_norm_comp, phenotype == "N")

info_table_num<-tibble("Age" , mean(ps_not_norm_time1_asd@sam_data$Age..years.), sd(ps_not_norm_time1_asd@sam_data$Age..years.), mean(ps_not_norm_time1_td@sam_data$Age..years.), sd(ps_not_norm_time1_td@sam_data$Age..years.), 1.544463e-01)

colnames(info_table_num) <- c("Variable", "Mean_(ASD)", "SD_(ASD)", "Mean_(TD)", "SD_(TD)", "q-value")

info_table_sex <-tibble("Sex" , sum(ps_not_norm_time1_asd@sam_data$Biological.sex == "Male"), sum(ps_not_norm_time1_asd@sam_data$Biological.sex == "Female"), sum(ps_not_norm_time1_td@sam_data$Biological.sex == "Male"), sum(ps_not_norm_time1_td@sam_data$Biological.sex == "Female"), 2.140230e-03)

colnames(info_table_sex) <- c("Variable", "Male_(ASD)", "Female_(ASD)", "Male_(TD)", "Female_(TD)", "q-value")

info_table_bool <-tibble("C-Section_Birth" , sum(ps_not_norm_time1_asd@sam_data$Born.by.caesarian.section == "TRUE"), sum(ps_not_norm_time1_asd@sam_data$Born.by.caesarian.section == "FALSE"),  sum(ps_not_norm_time1_td@sam_data$Born.by.caesarian.section == "TRUE"), sum(ps_not_norm_time1_td@sam_data$Born.by.caesarian.section == "FALSE"), 1.00, .name_repair = "unique")
colnames(info_table_bool) <- c("Variable", "TRUE_(ASD)", "FALSE_(ASD)", "TRUE_(TD)", "FALSE", "q-value")

#change to per timpoint
info_table_bool.tmp <-tibble("GI_within_Two_Weeks" , sum(ps_not_norm_comp_asd@sam_data$GI.issues.two.weeks.ago..M3. == "TRUE"), sum(ps_not_norm_comp_asd@sam_data$GI.issues.two.weeks.ago..M3. == "FALSE"),  sum(ps_not_norm_comp_td@sam_data$GI.issues.two.weeks.ago..M3. == "TRUE"), sum(ps_not_norm_comp_td@sam_data$GI.issues.two.weeks.ago..M3. == "FALSE"), 1.00, .name_repair = "unique")
colnames(info_table_bool.tmp) <- c("Variable", "TRUE_(ASD)", "FALSE_(ASD)", "TRUE_(TD)", "FALSE", "q-value")
info_table_bool <- rbind(info_table_bool.tmp, info_table_bool)

info_table_bool.tmp <-tibble("GI_within_Three_Months" , sum(ps_not_norm_time1_asd@sam_data$GI.symptoms.within.3.months..M3. == "TRUE"), sum(ps_not_norm_time1_asd@sam_data$GI.symptoms.within.3.months..M3. == "FALSE"),  sum(ps_not_norm_time1_td@sam_data$GI.symptoms.within.3.months..M3. == "TRUE"), sum(ps_not_norm_time1_td@sam_data$GI.symptoms.within.3.months..M3. == "FALSE"), 3.111005e-05, .name_repair = "unique")
colnames(info_table_bool.tmp) <- c("Variable", "TRUE_(ASD)", "FALSE_(ASD)", "TRUE_(TD)", "FALSE", "q-value")
info_table_bool <- rbind(info_table_bool.tmp, info_table_bool)
