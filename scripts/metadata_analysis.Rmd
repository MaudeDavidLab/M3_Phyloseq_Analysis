...
for (i in num_cat ){
  ...
  tmpmap <- map
  if (tmp$p.value <= 0.05) {
    if (sum(is.na(map[which(map$Within.study.sampling.date == "Timepoint 1"),i])) != 0){
    tmpmap <- map[-which(map$Family.group.ID %in% c(map$Family.group.ID[which(map$Within.study.sampling.date == "Timepoint 1")][is.na(map[which(map$Within.study.sampling.date == "Timepoint 1"),i])])),]
    }
    time_1<-subset(tmpmap, Within.study.sampling.date == "Timepoint 1")
    time_1_A <-subset(time_1, phenotype == "A")
    time_1_N <-subset(time_1, phenotype == "N")
    orderedforpairedwilcox<-rbind(time_1_A , time_1_N)
    
    res<-wilcox.test(as.formula(paste(i, "~ phenotype")), data=orderedforpairedwilcox, var.equal = FALSE, paired = TRUE)
    ...
    time_1_N <-subset(time_1, phenotype == "N")
    orderedforpairedwilcox<-rbind(time_1_A , time_1_N)
    res<-t.test(as.formula(paste(i, "~ phenotype")), data=orderedforpairedwilcox, paired = TRUE)
...
for (i in num_cat) {
  ...
  if (tmp$p.value <= 0.05) {
    ...
    tmpmap <- map[-which(map$fam_and_time %in% c(map$fam_and_time[is.na(map[,i])])),]
    }
    
    A <-subset(tmpmap, phenotype == "A")
    N <-subset(tmpmap, phenotype == "N")
    orderedforpairedwilcox<-rbind(A , N)
...
sig_numvar2<-numerical_test_btwn_pheno


sig_numvar<-rbind(sig_numvar2, sig_numvar)
sig_numvar$p.value <- p.adjust(sig_numvar$p.value)
