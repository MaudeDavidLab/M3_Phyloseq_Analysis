...
- Read in a phyloseq object containing raw ASV counts, a mapping file, and a taxonomy table (produced from Dada2).
- Declare 2 methods for normalization
- Call a separate script in order to clean up the mapping file. This process will rename columns to have shorter names, change survey answers into numerical values, and add the season that each sample was collected (based on the date)
- Remove samples that were self-reported to be breast fed and those that did not pass our phenotype validation screening (see manuscript)
- run Friedman test to remove taxa that vary dramatically in abundance across timepoints within the same individuals
- save resulting phyloseq objects for later use

```{r setup, include=FALSE, root.dir("C:/Users/kitikomp/Documents/Lab/M3_Phyloseq_Analysis")}
library(dplyr)
library(reshape2)
library(phyloseq)
library(rstatix)
...
library(DESeq2)
setwd("C:/Users/kitikomp/Documents/Lab/M3_Phyloseq_Analysis")
source("scripts/clean_mapping.R")
```
...
# Declare Normalization Functions
```{r normalization}
...
write.csv(sample_data(ps_not_norm_comp), "../data/sam_data.csv")
...
#Turn survey answers into numeric input
#Add season of sample collection
map <- clean_mapping_file(map)
...
#Remove Possible phenotype Contradictions and their families
...
# Normalize counts using css, deseq, and total sum scaling and save objects
```{r}
# CSS normalization
