...
setwd("~/Tree_M3_datasets/M3_Phyloseq_Analysis/scripts/")
library(caret)
...
```
### Metadata and 14 randomly selected taxa
```{r fig.height = 3, fig.width=5}
set.seed(1)
use <- sample(taxa_names(ps_deseq), 14)
perf_obj_rand <- rand_forest(pred_sequences = use, ps = ps_deseq, include_taxa = T, include_meta = T)
plotAUCs(perf_obj_rand, 3, "14 randomly selected taxa and metadata")
```
...
df <- data.frame('meta' = unlist(auc_meta), 'meta_bio' = unlist(auc_meta_bio), 'meta_null' = unlist(auc_meta_null))
ggplot(melt(df), aes(x = variable, y = value, color = variable)) + geom_boxplot() + geom_jitter(width = .2) + theme_bw()
```
