...
```
### Metadata and 11 randomly selected taxa distribution
...
nulldf<-data.frame('meta_null_dist_no_sig' = meta_null_nosig) #,  'meta_null_dist_no_db_sig' = meta_null_nodoublesig)
nulldf<-melt(nulldf)
auc_meta <- lapply(perf_obj_meta, function(x) return(x[[1]]))
auc_meta_bio <- lapply(perf_obj_biomarkers, function(x) return(x[[1]]))

df <- data.frame('meta' = unlist(auc_meta), 'meta_bio' = unlist(auc_meta_bio))
df<-melt(df)
df_all<-rbind(df, nulldf)

my_comparison <- list(c("meta", "meta_bio"), c("meta","meta_null_dist_no_sig"), c("meta_bio","meta_null_dist_no_sig"))#, #c("meta_bio","meta_null_dist_no_db_sig"), c("meta","meta_null_dist_no_db_sig"))

ggplot(df_all, aes(x = variable, y = value, color = variable)) + geom_boxplot() + geom_jitter(width = .2) + theme_bw() +stat_compare_means(comparisons = my_comparison, label = "p.signif", hide.ns =TRUE)+labs(title="Random Forest Model AUC value Comparison",x ="Predictors Used", y = "AUC", color = "Predictors Used")
```
