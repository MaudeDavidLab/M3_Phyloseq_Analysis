...
location<-read.csv("../data/M3_FamilyIDs_States.csv")

#file actually ends at 121
location<-location[1:121,]

location$M3_Family_ID_formatted <- gsub("Family_00", "", as.character(location$M3_Family_ID))
location$M3_Family_ID_formatted <- gsub("Family_0", "", as.character(location$M3_Family_ID_formatted))
location$M3_Family_ID_formatted <- gsub("Family_", "", as.character(location$M3_Family_ID_formatted))

ps_deseq@sam_data$state <- "NA"

for(i in 1:121){
  ps_deseq@sam_data$state[which(ps_deseq@sam_data$Family.group.ID == location$M3_Family_ID_formatted[i])] <- as.character(location$state[i])}

ps_deseq@sam_data$state <- as.factor(ps_deseq@sam_data$state)

map<-as.data.frame(sample_data(ps_deseq))

map$phenotype<-as.factor(map$phenotype)
map$Host.disease.status <- NULL

num_cat <- colnames(map)[unlist(lapply(map, is.numeric))]
num_cat <- num_cat[-which(num_cat %in% c("Number.of.small.pet.herbivores", "Number.of.small.pet.rodents", "phenotype_num", "Number.of.pet.birds", "Host.ID", "Biospecimen.ID", "NCBI.taxID", "LR10.probability.ASD..M3.", "LR5.probability.ASD..M3.", "LR6.probability.ASD..M3." , "LR10.prediction..M3.", "LR10.probability.not.ASD..M3.", "LR5.probability.not.ASD..M3." , "LR5.prediction..M3.", "LR6.prediction..M3." , "LR6.probability.not.ASD..M3."))]

#removed LR predictions since those are essentially an indicator of phenotype and not confounding variables
...
prep_perma_per_time <- function(t){

tmpmap<-map[which(map$Within.study.sampling.date == t), ]
  
#we keep only the cateory selected above as relevant 
tmp_metadata <- tmpmap[,c(num_cat,fac_cat)]
...
# filter out variables with n-4 all same responses across datset (140 out of 144 samples)
cols_keep <- as.vector(apply(metadata, 2, function(x) return(sum(table(x)[which(table(x) >= 140)] >= 140)))) == 0
metadata <- metadata[ , cols_keep]


ps_tmp <- ps_deseq
#PER TIMEPOINT
keep <- ps_tmp@sam_data$Within.study.sampling.date %in% t
ps_tmp <- prune_samples(keep, ps_tmp)

ps_tmp@sam_data <- metadata 
return(ps_tmp)
}

perma_time_1<-prep_perma_per_time("Timepoint 1")
perma_time_2<-prep_perma_per_time("Timepoint 2")
perma_time_3<-prep_perma_per_time("Timepoint 3")
...
#to the distances between samples in microbiome spaces
#PER TIMEPOINT due to between and ingroup effect with repeated measures

perma_beta_and_adonis <- function(ps){
set.seed(1)
...
res_1<-perma_beta_and_adonis(perma_time_1)
res_2<-perma_beta_and_adonis(perma_time_2)
res_3<-perma_beta_and_adonis(perma_time_3)

all_res<-rbind(res_1[which(res_1$pvalue_adj< 0.05),], res_2[which(res_2$pvalue_adj< 0.05),], res_3[which(res_3$pvalue_adj< 0.05),])

freq<-freq_table(all_res$Variable)

all_res<-all_res[which(all_res$Variable %in% freq$group[which(freq$n == 3)]),]

permanova_res <- NULL
for(i in unique(all_res$Variable)){
  tmp<-all_res[which(all_res$Variable == i),]
  tmptable<-tibble(unique(tmp$Variable), unique(tmp$FactorClass), sum(tmp$TotalN), mean(tmp$R2), sd(tmp$R2))
  colnames(tmptable) <- c("Variable", "FactorClass", "TotalN", "Mean R2", "Std. Dev")
  tmptable$`Mean R2` <- round(tmptable$`Mean R2`, digits = 3)
  tmptable$`Std. Dev` <- round(tmptable$`Std. Dev`, digits = 3)
  permanova_res<-rbind(permanova_res, tmptable)
}
...
permanova <- function(physeq, factorName, ifnumeric, pmt = 999){
  ...
  braydist = phyloseq::distance(physeq, "bray")
  form <- as.formula(paste("braydist ~ ", c(factorName), sep = ""))
  metaDF=data.frame(sample_data(physeq)[, c(as.character(factorName), "Within.study.sampling.date")])
  # if numerical variable, make sure the class
  if(ifnumeric){
    metaDF[, factorName] <- as.numeric(metaDF[, factorName])
    factor_class='numeric'
  ...
  perm <- adonis(form, strata = metaDF$Within.study.sampling.date, permutations = pmt, metaDF)
  permDT=data.table(Variable=factorName, 
             FactorClass=factor_class,
             TotalN=perm$aov.tab['Total','Df']+1, 
             R2=perm$aov.tab[factorName, 'R2'], 
...
#MARa variables
cols_away <- c(anxiety[-which(anxiety %in% "Recent.anxiety..caretaker.reported.")], learning, social, "Mobile.Autism.Risk.Assessment.Score")
metadata <- metadata[ , -which(colnames(metadata) %in% cols_away)]
...
plot_grid(plotlist = to_plot[c(1:3)], ncol = 2)
...
#only ASD
tmp_metadata<-tmp_metadata[tmp_metadata$phenotype == "A",]
...
#Test the homogeneity of dispersion before running permanova - we only wish to test variables that are dispersed homogeneously with respect
#to the distances between samples in microbiome space
run_behavior_perma_per_time<-function(t){
set.seed(1)
pval_factors_disper=c()
nb_samples_disper=c()
keep <- ps_deseq_ASD@sam_data$Within.study.sampling.date == t
ps <- prune_samples(keep, ps_deseq_ASD)
for (i in 1:ncol(metadata)){
  cat (i,"\t")
  #Drop samples that have NA for the specified variable
  test_map <- metadata[!is.na(metadata[,i]) & metadata[,i] != "" ,]
...
behav_res1<-run_behavior_perma_per_time("Timepoint 1")
behav_res2<-run_behavior_perma_per_time("Timepoint 2")
behav_res3<-run_behavior_perma_per_time("Timepoint 3")

#No sig
behav_all_res<-rbind(behav_res1[which(behav_res1$pvalue_adj< 0.05),], behav_res2[which(behav_res2$pvalue_adj< 0.05),], behav_res3[which(behav_res3$pvalue_adj< 0.05),])

behav_all_res

behav_nonsig_table <- rbind(behav_res1, behav_res2, behav_res3)

#freq<-freq_table(behav_all_res$Variable)

#behav_all_res<-behav_all_res[which(behav_all_res$Variable %in% freq$group[which(freq$n == 3)]),]

#permanova_res <- NULL
#for(i in unique(behav_all_res$Variable)){
#  tmp<-behav_all_res[which(behav_all_res$Variable == i),]
#  tmptable<-tibble(unique(tmp$Variable), unique(tmp$FactorClass), sum(tmp$TotalN), mean(tmp$R2), sd(tmp$R2))
#  colnames(tmptable) <- c("Variable", "FactorClass", "TotalN", "Mean R2", "Std. Dev")
#  tmptable$`Mean R2` <- round(tmptable$`Mean R2`, digits = 3)
#  tmptable$`Std. Dev` <- round(tmptable$`Std. Dev`, digits = 3)
...
datatable(behav_nonsig_table)
write.csv(behav_nonsig_table, file ="../results/MARA_var_perma.csv") 
...
#MARa variables
cols_keep <- c(anxiety[-which(anxiety %in% "Recent.anxiety..caretaker.reported.")], learning, social)
metadata <- tmp_metadata[ , c(cols_keep, "Mobile.Autism.Risk.Assessment.Score", "Within.study.sampling.date")]
...
permanova_res$R2 <- round(permanova_res$R2, digits = 3)
permanova_res$pvalue_adj <- round(permanova_res$pvalue_adj, digits = 3)
behav_all_res <- permanova_res

#10sig
behav_all_res<-behav_all_res[which(behav_all_res$pvalue_adj< 0.05),]

datatable(behav_all_res)

write.csv(behav_all_res, file ="../results/MARA_var_perma.csv") 

#determine if sig with "perfect" values in TD

tmp_metadata <- map[,c(num_cat,fac_cat)]

tmp_metadata$Repetitive.motion[which(is.na(tmp_metadata$Repetitive.motion))] <- 1
tmp_metadata$Sleep.pattern.finding[which(is.na(tmp_metadata$Sleep.pattern.finding))] <- 1
tmp_metadata$Response.to.typical.sounds[which(is.na(tmp_metadata$Response.to.typical.sounds))] <- 1
tmp_metadata$Self.injurious.behavior.finding[which(is.na(tmp_metadata$Self.injurious.behavior.finding))] <- 1

tmp_metadata$Understands.speech[which(is.na(tmp_metadata$Understands.speech))] <- 4
tmp_metadata$Childhood.behavioral.development.finding[which(is.na(tmp_metadata$Childhood.behavioral.development.finding))] <- 4
tmp_metadata$Language.ability.and.use[which(is.na(tmp_metadata$Language.ability.and.use))] <- 4
tmp_metadata$Conversation.ability[which(is.na(tmp_metadata$Conversation.ability))] <- 4
tmp_metadata$Plays.imaginatively.when.alone[which(is.na(tmp_metadata$Plays.imaginatively.when.alone))] <- 4
tmp_metadata$Plays.imaginatively.with.others[which(is.na(tmp_metadata$Plays.imaginatively.with.others))] <- 4
tmp_metadata$Plays.in.a.group.with.others[which(is.na(tmp_metadata$Plays.in.a.group.with.others))] <- 4
tmp_metadata$Eye.contact.finding[which(is.na(tmp_metadata$Eye.contact.finding))] <- 4
tmp_metadata$Picks.up.objects.to.show.to.others[which(is.na(tmp_metadata$Picks.up.objects.to.show.to.others))] <- 4
tmp_metadata$Imitation.behavior[which(is.na(tmp_metadata$Imitation.behavior))] <- 4
tmp_metadata$Mobile.Autism.Risk.Assessment.Score[which(is.na(tmp_metadata$Mobile.Autism.Risk.Assessment.Score))] <- 3
...
### Metagenomeseq function for permanova variables
```{r}
...
#Will add in age 
perma_var <- c(as.character(perma_var), "Age..months.")


for(var in perma_var[1:length(perma_var)]){
  print(var)
  tmp <- run_metagenom_seq(ps_no_norm_filt, variable = as.character(var), 30, .05, time = T)
  print(paste0("taxa=",nrow(tmp[[2]])))
  print(paste0("totalgroup0counts", sum(tmp[[1]]$`+samples in group 0`)))
  print(paste0("totalgroup1counts", sum(tmp[[1]]$`+samples in group 1`)))
  if (nrow(tmp[[2]]) !=0){
...
ord_pcoa <- ordinate(
  ...
  method = "CAP", 
  distance = "bray",
  formula = as.formula(paste0("~", title_prep[i])))
  ...
  geom_point( size = 0.5) +
  theme(text = element_text(size =12), plot.title = element_text(size=15))
}
...
pdf(paste0(output_data,"behavioral_measures_pcoa.pdf"), width=16,height=40, title = "Supplementary File 12: Constrained PCOAs of Significant Behavior Variables")
grid.arrange(grobs = as.list(to_plot), layout_matrix = lay)
dev.off
...
## Ordinate by unifrac or weighted unifrac

```{r}

tree<-readRDS("../Tree/tree_rootbreastfedout")

#ps_deseq_tree <- ps_deseq

#phy_tree(ps_deseq_tree) <- tree


#Ordinating using Unifrac distances
#ps_pcoa = ordinate(ps_deseq_tree , 
#                    method="PCoA", 
#                    distance="unifrac")

# Plot the created ordination, we'll color it using the "color=" option
#plot<- plot_ordination(ps_deseq_tree , 
#                                   ps_pcoa, 
#                                   color= "Phenotype") +
#     ggtitle("PcoA using Unifrac distances") + theme(axis.text=element_text(size=14), axis.title=element_text(size=14), title = element_text(size=13), legend.text = #element_text(size = 18))

#plot

#DEseq2 normalization causes 0s during unifrac distance calculation, which causes error in ordination
#phyloseq::distance(ps_deseq_tree, method = "unifrac")

ps_css <- readRDS(paste0(output_data, "Filtered/ps_css_friedfilt.rds"))
ps_css_tree <- ps_css

phy_tree(ps_css_tree) <- tree

#Ordinating using Unifrac distances

permav_res<-read.csv("../filtered_perm_res.csv")
sigvars<-as.character(permav_res$Variable)

cols_keep <- c(sigvars, "phenotype")

tmp_metadata <- map[,c(num_cat,fac_cat)]
metadata <- tmp_metadata[ , which(colnames(tmp_metadata) %in% cols_keep)]

#For plots
metadata$Phenotype <- gsub("A", "ASD", metadata$phenotype)
metadata$Phenotype <- gsub("N", "TD", metadata$Phenotype)
...
sample_data(ps_css_tree) <- metadata
...
for (i in 1:length(title_prep)){
  ...
  geom_point( size = 0.3) +
  theme(text = element_text(size =6), plot.title = element_text(size=6), legend.title = element_blank()) 
}



multi.page <- ggpubr::ggarrange(plotlist = to_plot, nrow = 2, ncol = 2) 
ggpubr::ggexport(multi.page, filename = "../results/Supp_pcoa_unweighted_unifrac.pdf")
...
multi.page <- ggpubr::ggarrange(plotlist = to_plot, nrow = 2, ncol = 2) 
ggpubr::ggexport(multi.page, filename = "../results/Supp_pcoa_weighted_unifrac.pdf")
