...
num_cat <- num_cat[-which(num_cat %in% c("Language.ability.and.use", "Conversation.ability", "Understands.speech", "Number.of.small.pet.herbivores", "Number.of.small.pet.rodents", "phenotype_num", "Number.of.pet.birds", "Host.ID", "Biospecimen.ID", "NCBI.taxID" ))]
...
permanova_res <- permanova_res[permanova_res$pvalue < 0.05,]
ps_no_norm_filt <- readRDS(paste0(output_data, "Filtered/ps_no_norm_friedfilt.rds"))
taxa_associated_w_variables <- list()
#Skip Host.interaction.with.pets due to high NA and host.disease status since we are looking for potential confoudns
perma_var<-permanova_res$Variable[-which(permanova_res$Variable %in% c("Host.interaction.with.pets"))]
...
addConfounders <- function(sig_taxa, taxa_ass_w_var){
  ...
  for(asv in rownames(sig_taxa)){
    associated_variables <- rownames(taxa_ass_w_var[taxa_ass_w_var$ASV ==asv, ])
