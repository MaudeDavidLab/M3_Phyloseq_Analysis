...
ps_DeSeq_norm_pass_min_postDD_sup003_full <-ps_deseq
ps_DeSeq_norm_pass_min_postDD_sup003<-ps_deseq
```

## Comparison of Variance between Subjects and between Phenotypes averaging distances between all pairs of timepoints
```{r variance}

#Comparing variance w/ avg difference in distance
tmpps <- prune_samples((ps_deseq@sam_data$Family.group.ID == "1"), ps_deseq) #why are we taking out family 1? It is to make the first table to rbind to in my next for loop, its a pretty inefficient way of building my table haha
...
ave_distanceA = mean(c(A[1],A[2],A[3]))

N <- distance(tmppsN, "bray", type = "samples")
ave_distanceN <- mean(c(N[1],N[2],N[3]))
...
for(i in unique(ps_deseq@sam_data$Family.group.ID)[2:length(unique(ps_deseq@sam_data$Family.group.ID))]){
